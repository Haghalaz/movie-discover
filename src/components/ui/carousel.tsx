...
const CarouselNav = () => {
  ...
  return (
    <div className="my-4 flex w-full items-center justify-center gap-2">
      {Array.from({ length: api?.scrollSnapList().length || 0 }, (_, i) => (
        <span
          key={i}
          className={`size-2 origin-center rounded-full bg-white transition-all ${i === api?.selectedScrollSnap() ? 'w-6 opacity-100' : 'opacity-20'}`}
        />
      ))}
    </div>
  );
...
export { type CarouselApi, Carousel, CarouselContent, CarouselNav, CarouselItem, CarouselPrevious, CarouselNext };
